...
results energies_and_forces(state molA, state molB, double &Lx, double &Ly, double &beta, bool abcd)
{
    ...
    if ((molA.damping_coeff == 0) || (molB.damping_coeff == 0))
    {
  	...
  	double dist_x, dist_y;	// Distance between A and B molecules along x and y axies
		...
		// molA's charge coordinates are NOT precomputed by the structure builder
		// (and the numerical path never needed them), so compute them here. Without
		// this the Coulomb term is evaluated against zeroed charges, leaving only the
		// repulsive LJ part, and the crystal disperses. molB's charges are computed
		// per periodic image below.
		charges_coordinates(molA);
