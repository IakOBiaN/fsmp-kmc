"""Tab 1: build, edit, load and save the molecule; attach it to the project.

A project holds exactly one molecule; "Use in project" saves the edited
molecule into the project folder and makes it the project molecule.
"""
...
class MoleculeTab(QWidget):
    ...
    def __init__(self, project: Project, parent=None):
        ...
        self.canvas.atomAddRequested.connect(self._add_atom_from_canvas)
        ...
        self._refresh_project_molecule()
        self._update_info()
    ...
    def _build_side_panel(self) -> QWidget:
        ...
        caption2 = QLabel("Project molecule")
    ...
    def _refresh_project_molecule(self) -> None:
        entry = self.project.molecule
        if entry is None:
    ...
    def use_in_project(self) -> None:
        ...
        if not ok or not name.strip():
            return
        name = name.strip()
        old = self.project.molecule
        if old is not None:
            answer = QMessageBox.question(
                self, "Replace project molecule",
                f"The project molecule '{old['name']}' will be replaced. Continue?")
            if answer != QMessageBox.Yes:
                return
        self.project.set_molecule(name, mol)
        self._current_name = name
        self._dirty = False
        self._refresh_project_molecule()
        self.statusMessage.emit(f"'{name}' is now the project molecule")
    ...
    def _open_project_molecule(self) -> None:
        ...
        if not self._confirm_discard():
            return
        self._load_file(str(self.project.molecule_path(entry)), name=entry["name"])
    ...
    def _clear_project_molecule(self) -> None:
        ...
        if entry is None:
            return
        answer = QMessageBox.question(
            self, "Remove project molecule",
            f"Remove '{entry['name']}' from the project?\n"
            "The xyz file inside the project folder will be deleted.")
        if answer != QMessageBox.Yes:
            return
        self.project.clear_molecule()
        self._refresh_project_molecule()
        self.statusMessage.emit("Project molecule removed")
